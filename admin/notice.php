...
function spt_rating_admin_notice() {
    ...
    $dismiss = wp_nonce_url( add_query_arg( 'spt_rating_notice_action', 'spt_dismiss_rating_true' ), 'spt_dismiss_rating_true' ); 
    $no_thanks = wp_nonce_url( add_query_arg( 'spt_rating_notice_action', 'spt_no_thanks_rating_true' ), 'spt_no_thanks_rating_true' ); ?>
...
function spt_dismiss_rating_admin_notice() {
    ...
    if ( 'spt_dismiss_rating_true' === $_GET['spt_rating_notice_action'] ) {
        check_admin_referer( 'spt_dismiss_rating_true' );
        update_option( 'spt_plugin_dismiss_rating_notice', '1' );
    }

    if ( 'spt_no_thanks_rating_true' === $_GET['spt_rating_notice_action'] ) {
